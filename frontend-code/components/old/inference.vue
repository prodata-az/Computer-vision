                ...
                <div class="video-container__block">
                  <div class="video-container__parts">
                    ...
                    <div class="second-item">
                      <div class="clusters">
                        <div class="clusters__block">
                          <ul class="clusters__list">
                            <li
                              v-for="(cluster, idx) in clusterList"
                              @click="currentClusterIndex = idx"
                            >
                              <UXButton
                                :class="{
                                  'btn-active': currentClusterIndex === idx,
                                }"
                              >
                                {{ `Cluster ${idx + 1}` }}
                              </UXButton>
                  ...
                  <div class="video-container__parts">
                    <div class="first-item">
                      ...
                      >
                        <div
                          v-if="inference.videoPaths.length"
                          class="videos-list"
                        >
...
<script setup>
  ...
  const change = async (event) => {
    try {
      ...
      await inference.getVideoPaths();

      // await inference.getHardLabels();

      const run_id = 25;

      await inference.getLogs(run_id);

      const objectofLabels = Object.values(inference.logs);

      objectofLabels.forEach((element) => {
        const arr = Object.values(element);
        inference.resultLabels.push(arr);
      });

      inference.createDataForChart();

      // add key for all_values
      inference.startPoints.forEach((element, index) => {
        all_values.value[index] = { timestamp: element, labels: [] };
        all_values.value[index].labels = inference.resultLabels[index];
      });
      ...
      console.log(inference.resultLabels);
