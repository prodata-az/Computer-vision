...
def split_by_seconds(filename, split_length, folder_name = "./", vcodec="copy", acodec="copy",
    ...
    for n in range(0, split_count):
        ...
        if n == 0:
            split_start = 0
        elif n == split_count - 1 :
            split_start = video_length - split_length - 2
        else:
            split_start = split_length * n
        split_cmd = ["ffmpeg", "-ss", str(split_start), "-i", filename, "-t", str(split_length + 2), "-vcodec", vcodec, "-acodec", acodec, folder_name+ filebase + "_" + str(n + 1) + ".mp4"] + shlex.split(extra)
...
def main():
    ...
    parser.add_option("-o", "--output",
                      dest="foldername",
                      default = "cut_videos/",
                      help="folder to save, for example splitted",
                      type="string",
                      action="store"
                      )
